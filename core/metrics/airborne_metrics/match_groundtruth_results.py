...
def run(flags):  
    ...
    df_results = df_results.merge(df_gt[['flight_id','frame','img_name']].drop_duplicates(), 
                                                                on='img_name', how='left') # add flight id for track results
    # TODO: remove below when Sleipnir dataset is fixed to have 306 flights
    df_results = df_results.dropna(subset=['flight_id']) # 删除没有flight_id的那一行
    log.info('Augmenting with track length')
    df_results = augment_with_track_len(df_results) # add track len
