...
from .utils import embedding_distance, iou_distance, linear_assignment,centerdist,area_dist,auto_dist
...
class EmbeddingTracker(object):
    ...
    def update(self, all_det_data):
        ...
        dists = centerdist(dists,strack_pool,tem_detections,self.spatial_dist)
        
        # dists = auto_dist(dists,strack_pool,tem_detections,self.spatial_dist,self.frame_id)
    ...
    def cascade_update(self,all_det_data):
        self.frame_id += 1
        # 每一次matching的的所有临时状态：
        # 四个track状态 一个检测状态
        tem_activated_stracks, tem_refind_stracks, tem_lost_stracks, tem_removed_stracks, tem_detections = [], [], [], [], []
        ...
        for det_data in all_det_data:
            tem_detections.append(STrack(det_data["bbox_tlwh"], np.asarray(det_data["conf"]), np.asarray(det_data["emb"]), self.feat_alpha))
            tem_scores.append(det_data["conf"])
        
        # order det result according det score
        scores_max2min = sorted(tem_scores,reverse=True)
        match_orders = [tem_scores.index(score) for score in scores_max2min]
        ...
        for temp_det_order in match_orders:
            dists = embedding_distance(strack_pool, [tem_detections[temp_det_order]])

            dists = centerdist(dists,strack_pool,tem_detections,self.spatial_dist)
            
            # 线性匹配,距离的阈值上限,大于阈值后置为inf,默认inf
            matches, u_track, _ = linear_assignment(dists, thresh=self.linear_assignment_dist)
            # 匹配到的track和det的基础上：
            for itracked, idet in matches:
                track = strack_pool[itracked]
                det = tem_detections[idet]
        ...
        """ Step 4: Init new stracks"""
        if not match_flag and len(tem_detections):
            track = tem_detections[match_orders[0]]
            if track.score >= self.det_thresh:
                track.activate(self.frame_id, self.next_count(new_id=True))
                tem_activated_stracks.append(track)
